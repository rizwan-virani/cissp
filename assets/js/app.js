/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `CISSP` namespace with
...
(function () {
  ...
  var PASS = EX.passing;                    // passing scaled score (from CISSP.exam)
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("cissp." + k, fb); },
      set: function (k, v) { write("cissp." + k, v); },
      del: function (k) { try { localStorage.removeItem("cissp." + k); } catch (e) {} }
    };
  })();
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the CISSP is scored and delivered as a Computerized Adaptive Test, and where the credential fits in a senior security career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and the adaptive question engine for the CISSP &mdash; the high-stakes details every candidate should internalize before sitting the test.",
    ...
    g1.appendChild(card({
      ...
      desc: "Where CISSP sits on the career ladder: the enterprise security-management and engineering judgment it certifies, and the security manager, architect, and CISO-track roles it unlocks.",
    ...
    b2.appendChild(el("p.sectsub", { text: "Eight domains, each a rigorous reading interface with deep technical definitions, comparison tables, exam tips, and real-world scenarios." }));
    var g2 = el("div.grid");
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every CISSP acronym across all eight domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    b4.appendChild(el("p.sectsub", { html: "Drawn at random from a bank of <b>" + (S.questions ? S.questions.length : "800+") + "</b> original items, graded with full rationale. Domain quizzes isolate one objective set; the quick quiz mixes all eight." }));
    ...
    g3b.appendChild(card({
      ...
      desc: "Ten questions drawn uniformly from all eight domains &mdash; a fast, mixed knowledge check with full rationale on every item.",
    ...
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the CISSP &mdash; " + EX.maxQuestions + " questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: the official ISC2 CISSP exam outline, canonical study guides, MindMap video walkthroughs, and trusted community resources.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.CISSP_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function renderCertCatalog(mount) {
    ...
    var search = el("input.cert-search", { type: "text", placeholder: "e.g. CISSP" });
  ...
  function openLabSandbox(lab, i) {
    ...
    function runCommand(raw) {
      ...
      if (low === "whoami") { logLine("student (Security Professional)"); return; }
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates CISSP.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.CISSP_VER ? "?v=" + window.CISSP_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.replace(readerView("Domain " + id, d.title, "CISSP Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: CISSP", "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where CISSP fits in your path", S.careerGuidance || [])); }
  ...
  /* ===========================================================================
     BOOT
     =========================================================================== */
  /* Populate the hero exam-line from CISSP.exam so the figures live in exactly
