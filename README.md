...
**A comprehensive, open-source ISC2 CISSP study platform.**
In-depth coverage of all 8 exam domains, a 800-question bank with full per-distractor rationale, 800 Rapid Recall flashcards, 30 performance-based decision simulations, a drag-and-drop taxonomy classifier, 20 hands-on practice labs, and a full-length timed mock.
...
| Area | What you get |
| --- | --- |
| **Domain study modules** | 8 rigorous reading interfaces, one per exam domain, with deep technical definitions, comparison tables, exam tips, and real-world scenarios. **84 sections** mapped to the official CISSP outline, lazy-loaded per domain. |
| **Rapid Recall flashcards** | **800 cards** with a **Leitner spaced-repetition** scheduler, per-domain decks, a Master Acronym Drill, and an All Due Today review. |
| **Practice quizzes** | A bank of **800 original questions**, each graded with a full rationale that explains the correct answer **and why every individual distractor is wrong**. Domain, quick, adaptive, and missed-question modes. |
...
1. **Read the Exam Mechanics & Career Guidance cards first** to understand the adaptive format, scoring, and the "think like a manager" mindset.
...
└── assets/
    ...
    └── js/
        ...
        └── content/            # lazy-loaded modules
            ├── domain1..8.js      # dense reading per domain
            ├── flash1..8.js       # flashcard decks
            ├── quiz1..8.js        # question bank per domain
